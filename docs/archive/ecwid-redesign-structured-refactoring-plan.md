...
> **DEPRECATED — DO NOT IMPLEMENT (archived June 2026).**
> This plan was superseded by the Housecall Pro (HCP) design system. The canonical palette lives in `packages/design-tokens/src/tokens.ts` and `www.housecallpro.com-DESIGN.md`. CTAs use **gold** (`#FF9B24`), not Ecwid green. Kept for historical reference only.
