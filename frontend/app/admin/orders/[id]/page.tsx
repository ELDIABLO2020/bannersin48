...
export default function AdminOrderWorkspacePage() {
  ...
  if (detail.isLoading) return <p className="text-ink-muted">Loading order…</p>;
  ...
  return (
    <div className="space-y-xl">
      <div className="flex flex-wrap items-start justify-between gap-md">
        <div>
          ...
          <div className="flex items-center gap-sm mt-xs">
            ...
            <Badge variant={order.status === "DELIVERED" ? "success" : order.status === "CANCELLED" ? "error" : "info"}>{order.status}</Badge>
            {order.slaBreached && <Badge variant="error">Past SLA</Badge>}
          </div>
          <p className="text-body-sm text-ink-muted">{order.customer.email} · Payment: {order.paymentStatus}</p>
        </div>
      ...
      {message && <div className={`rounded-feature p-md text-body-sm ${message === "Saved." ? "bg-success-bg text-success-fg" : "bg-badge-error-bg text-danger"}`}>{message}</div>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-xl">
        <section className="lg:col-span-8 space-y-lg">
          ...
          <Card className="bg-surface p-lg">
            ...
            <ol className="space-y-sm">
              {order.events.map((event) => <li key={event.id} className="flex gap-md text-body-sm"><time className="w-40 shrink-0 text-ink-muted">{new Date(event.createdAt).toLocaleString()}</time><div><span className="font-bold text-ink">{event.toStatus}</span>{event.note && <span className="text-ink-muted"> · {event.note}</span>}</div></li>)}
            </ol>
          </Card>
        </section>
        ...
        <aside className="lg:col-span-4 space-y-lg">
          <Card className="bg-surface p-lg">
            ...
            <ol className="space-y-lg">
              <Step number="1" title="Payment received" complete={order.paymentStatus !== "PENDING_PAYMENT"}>
                {order.paymentStatus === "PENDING_PAYMENT" && <Button disabled={mutation.isPending} onClick={() => mutation.mutate(() => getAdminApiClient().markPaid(id))} className="w-full">Mark paid</Button>}
              </Step>
              <Step number="2" title="Drop-ship submission" complete={Boolean(order.dropship)}>
                {order.dropship ? <p className="text-body-sm text-ink-muted">Ref {order.dropship.externalRef}</p> : <div className="space-y-sm"><Input placeholder="External reference" value={externalRef} onChange={(e) => setExternalRef(e.target.value)} /><Input placeholder="Notes (optional)" value={dropshipNotes} onChange={(e) => setDropshipNotes(e.target.value)} /><Button variant="secondary" disabled={!externalRef || mutation.isPending} onClick={() => mutation.mutate(() => getAdminApiClient().recordDropship(id, { externalRef, notes: dropshipNotes }))} className="w-full">Record submission</Button></div>}
              </Step>
              <Step number="3" title="Tracking & label" complete={Boolean(order.shipment?.trackingNumber)}>
                {order.shipment?.trackingNumber ? <div className="text-body-sm text-ink-muted"><p>{order.shipment.trackingNumber}</p>{order.shipment.labelDownloadUrl && <a href={authorizedUrl(order.shipment.labelDownloadUrl)} className="text-link" target="_blank" rel="noreferrer">Download label</a>}</div> : <div className="space-y-sm"><Input placeholder="FedEx tracking number" value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} /><input aria-label="Label PDF" type="file" accept="application/pdf" className="text-xs text-ink-muted" onChange={(e) => setLabel(e.target.files?.[0])} /><Button variant="secondary" disabled={trackingNumber.length < 6 || mutation.isPending} onClick={() => mutation.mutate(() => getAdminApiClient().attachTracking(id, { trackingNumber, label }))} className="w-full">Attach tracking</Button></div>}
              </Step>
              <Step number="4" title="Ship & deliver" complete={order.status === "DELIVERED"}>
                <div className="flex flex-wrap gap-xs">
                  {order.status === "ACCEPTED" && <Button size="sm" disabled={mutation.isPending} onClick={() => mutation.mutate(() => getAdminApiClient().transition(id, { status: "SHIPPED", reason }))}>Mark shipped</Button>}
                  {order.status === "SHIPPED" && <Button size="sm" disabled={mutation.isPending} onClick={() => mutation.mutate(() => getAdminApiClient().transition(id, { status: "DELIVERED", reason }))}>Mark delivered</Button>}
                </div>
              </Step>
            </ol>
          </Card>
          ...
          {!['DELIVERED','CANCELLED'].includes(order.status) && <Card className="bg-surface p-lg"><h2 className="font-bold text-ink mb-sm">Exception controls</h2><Input placeholder="Reason / note" value={reason} onChange={(e) => setReason(e.target.value)} /><div className="flex gap-sm mt-sm"><Button variant="secondary" size="sm" disabled={mutation.isPending} onClick={() => mutation.mutate(() => getAdminApiClient().transition(id, { status: "ON_HOLD", reason: reason || "Placed on hold by staff." }))}>Put on hold</Button><Button variant="secondary" size="sm" className="border-danger text-danger" disabled={!reason || mutation.isPending} onClick={() => mutation.mutate(() => getAdminApiClient().transition(id, { status: "CANCELLED", reason }))}>Cancel</Button></div></Card>}
        </aside>
      </div>
    </div>
  );
...
function Step({ number, title, complete, children }: { number: string; title: string; complete: boolean; children: React.ReactNode }) { return <li><div className="flex items-center gap-sm mb-sm"><span className={`h-6 w-6 rounded-full text-xs font-bold flex items-center justify-center ${complete ? "bg-success-bg text-success-fg" : "bg-surface-tint text-ink-muted"}`}>{complete ? "✓" : number}</span><span className="font-bold text-ink">{title}</span></div><div className="pl-8">{children}</div></li>; }
