...
api_json "$API/auth/me" -H "Authorization: Bearer $TOKEN" | jq -e '.email == "'"$EMAIL"'"' >/dev/null || fail "auth/me"
pass "GET /auth/me returns the profile"

CATALOG="$(api_json "$API/catalog/banner")"
[[ "$(echo "$CATALOG" | jq 'length')" == "7" ]] || fail "catalog expected 7 hub products"
api_json "$API/catalog/banner/$(echo "$CATALOG" | jq -r '.[0].slug')" \
  | jq -e '.id == "HD_BANNER" and (.commonUses | length > 0)' >/dev/null || fail "catalog detail"
pass "Catalog lists 7 hub products with HD Banner detail"
